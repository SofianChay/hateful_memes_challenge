...
def create(data, datatype, batch_size, images_features_dict):
    ...
    max_len_features = max([len(features)
                            for features in images_features_list])
    attention_masks_images = []

    for i in range(len(images_features_list)):
        attention_masks_images.append(
            [1] * len(images_features_list[i]) + [0] * (max_len_features - len(images_features_list[i])))
        images_features_list[i] = torch.cat((images_features_list[i], torch.zeros(
            (max_len_features - len(images_features_list[i]), 2048))), dim=0)

    for i in range(len(images_features_list)):
        input_ids[i] += [config.pad_token_id] * (max_len - len(input_ids[i]))
        attention_masks[i] += [0] * \
            (max_len - len(attention_masks[i])) + attention_masks_images[i]

    dataset = MyDataset(input_ids, attention_masks,
                        images_features_list, labels, ids)
